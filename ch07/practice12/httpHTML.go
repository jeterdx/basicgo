...
//HTML Templateを使えるようにする
var tpl *template.Template

func init() {
	tpl = template.Must(template.ParseFiles("out.html"))
}

//HTML出力するように書き換える、HTMLにmapを渡してあげる
func (db database) list(w http.ResponseWriter, req *http.Request) {
	err := tpl.Execute(w, db) // 変数をdbに指定、Executeの第一引数はio.Writer、 ResponsWriterはio.Writeインタフェースを満たしているので使える。
	if err != nil {
		log.Fatalln(err)
	}
}

//以下、他のメソッドのHTML対応はまた今度。。
